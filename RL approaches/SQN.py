...
class QNetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain, model='GRU', name='DQNetwork'):
        ...
        with tf.variable_scope(self.name):
            ...
            self.qa = trfl.indexing_ops.batched_index(self.output1, self.actions)
...
def evaluate(sess, model='GRU', test=False):
    ...
    while evaluated<len(eval_ids):
        ...
        for i in range(batch):
            ...
            evaluated+=1
        prediction, qtable, st, qa=sess.run([QN_1.output2, QN_1.output1, QN_1.states_hidden, QN_1.qa], feed_dict={QN_1.inputs: states,QN_1.len_state:len_states,QN_1.is_training:False,QN_1.actions:actions})
        if evaluated <= 500:
    ...
    # Save pikles for feature analysis 
    concat_preds = np.concatenate(qtables, axis=0)
    concat_states = np.concatenate(hiddens, axis=0)
    pca = PCA(n_components=80)  # reduce dimension
    reduced_data = pca.fit_transform(concat_preds)
    with open(os.path.join(data_directory, '%s_TH_preds_%f.pkl' % (model, args.discount)), 'wb') as handle:
        pkl.dump(reduced_data, handle, protocol=pkl.HIGHEST_PROTOCOL)
    with open(os.path.join(data_directory, '%s_TH_preds_states_%f.pkl' % (model, args.discount)), 'wb') as handle:
        pkl.dump(concat_states, handle, protocol=pkl.HIGHEST_PROTOCOL)
    with open(os.path.join(data_directory, '%s_TH_preds_vals_%f.pkl' % (model, args.discount)), 'wb') as handle:
        pkl.dump(vals, handle, protocol=pkl.HIGHEST_PROTOCOL)
    print('#############################################################')
