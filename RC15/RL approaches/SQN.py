...
def evaluate(sess, model='GRU', test=False):
    ...
    with open(os.path.join(data_directory, '%s_TH_preds.pkl' % model), 'wb') as handle:
        pkl.dump(reduced_data, handle, protocol=pkl.HIGHEST_PROTOCOL)
    with open(os.path.join(data_directory, '%s_TH_preds_states.pkl' % model), 'wb') as handle:
        pkl.dump(concat_states, handle, protocol=pkl.HIGHEST_PROTOCOL)
    with open(os.path.join(data_directory, '%s_TH_preds_vals.pkl' % model), 'wb') as handle:
        ...
        with tf.Session() as sess:
            ...
            for i in range(args.epoch):
                for j in range(num_batches):
                    ...
                    #     evaluate(sess)
            print("Evaluating on Test Data..., qloss {}", mod)
