...
class QNetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain, model='GRU', name='DQNetwork'):
        ...
        with tf.variable_scope(self.name):
            ...
            self.output1 = tf.contrib.layers.fully_connected(self.states_hidden, 6,
                                                             activation_fn=None)  # category prediction
            ...
            self.category = tf.placeholder(tf.int32, [None]) # Integer representing the associated category, refer to the paper for details.

            catloss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.category, logits=self.output1) # ce loss for category prediction
            celoss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.actions, logits=self.output2) # ce loss for item prediction
            self.loss = tf.reduce_mean(self.weight*catloss + celoss)
        ...
        with tf.Session() as sess:
            ...
            for i in range(args.epoch):
                for j in range(num_batches):
                    ...
                    fut_reward = list(batch['fut_reward'].values())
                    categor2 = list(batch['category'].values()) # Two-step category assignation.
                    ...
                    #     evaluate(sess)
            print("Evaluating on Test Data... CAT", mod)
