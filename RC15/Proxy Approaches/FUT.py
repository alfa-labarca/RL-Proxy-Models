    ...
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain, model='GRU', name='DQNetwork'):
        ...
        with tf.variable_scope(self.name):
            ...
            self.output1 = tf.contrib.layers.fully_connected(self.states_hidden, self.item_num,
                                                            activation_fn=None)  # future length predictions
            ...
            self.fut = tf.placeholder(tf.float32, [None]) # future length values


            qa_tm1 = trfl.indexing_ops.batched_index(self.output1, self.actions) # FUT prediction value for corresponding actions
            td_error = self.fut - qa_tm1
            futloss = 0.5 * tf.square(td_error) #MSE calculation
            celoss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.actions, logits=self.output2) #ce loss
            self.loss = tf.reduce_mean(self.weight * futloss + celoss)
...
if __name__ == '__main__':
    ...
    data_statis = pd.read_pickle(
        os.path.join(data_directory, 'data_statis_mio.df'))  # read data statistics, includeing state_size and item_num
        ...
        with tf.Session() as sess:
            ...
            for i in range(args.epoch):
                for j in range(num_batches):
                    ...
                    fut = list(batch['fut'].values()) # Future length values
                    ...
                    #     evaluate(sess)
            print("Evaluating on Test Data... fut length", mod)
